...
const CodeShareInvoiceApprove = ({selectedApp}:{selectedApp:string}) => {
  const [form] = Form.useForm();
  const [glFileList, setGLFileList] = useState<UploadFile[]>([]);
  const [itRawFileList, setItRawFileList] = useState<UploadFile[]>([]);
  const [reportType, setReportType] = useState<string>('paxinv-cs-reports');
  ...
  const getReports = async (value?: string, month?: string) => {
    try {
      setIsLoading(true)
      const res = await api.get(`https://finance-platform-blob-service-finance-platform-core-dev.apps.arolab37nonprod.goindigo.in/execute/listBlobs?containerName=${value}`)
      const res2 = await api.get(`https://finance-platform-blob-service-finance-platform-core-dev.apps.arolab37nonprod.goindigo.in/execute/listBlobs?containerName=paxinv-cs-processing`)
      ...
      if (res.status === 200) {
        ...
        res?.data.forEach((item: string, index: number) => {
          ...
          const data: DataType = {
            ...
            fileUrl: value === 'PROCESSING' ? '' : `https://finance-platform-blob-service-finance-platform-core-dev.apps.arolab37nonprod.goindigo.in/execute/download?blobName=${item}&containerName=${value}`,
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      setIsLoading(true)
      const res = await api.post(`https://finance-platform-blob-service-finance-platform-core-dev.apps.arolab37nonprod.goindigo.in/execute/upload?month=${selectMonth}`, formData)
      // console.log(res);
      if (res.status === 200) {
  ...
  const DownloadTab = () => <ContentSection >
    <div className='tab-buttons'>
      ...
      <Radio.Group value={reportType} onChange={handleReportList}>
        ...
        <Radio.Button value="paxinv-cs-reports">Report</Radio.Button>
        <Radio.Button value="paxinv-cs-archive">Archive</Radio.Button>
