...
const ErrorCorrections = ({ onNext, current }: UploadInterfaceProps) => {
  ...
  const handleFileUpload = async (values: any, filedata: any, form:any, setErrorFile:any) => {
    try {
      setIsLoading(true)
      const res = await api.post(`https://fp-cs-endpoints-svc-finance-platform-core-dev.apps.arolab37nonprod.goindigo.in/api/v1/uploadfile?fileType=${values.fileType}`, filedata, {
        headers: {
  ...
  const handleFileDownload = async (values: any, form:any) => {
    ...
    try {
      setIsLoading(true)
      const res = await api.get(`https://fp-cs-endpoints-svc-finance-platform-core-dev.apps.arolab37nonprod.goindigo.in/api/v1/downloadReport?fileType=${values.fileType}&startDate=${dayjs(values.startDate).format('MM/DD/YYYY')}&endDate=${dayjs(values.endDate).format('MM/DD/YYYY')}`, {
  ...
  <PageTitle
      ...
      //onBack={() => null}
      title="Upload / Download Error Report"
    />
